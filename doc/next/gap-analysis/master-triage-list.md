...
- [ ] Capture **Kodi** evidence for browse / hub / provider / search / info journeys and append to [Phase 07 playbook §Validation status](../roadmap/phase-07-stabilization-and-freeze.md#validation-status-and-blockers), per [kodi-complete-testing guide](../../context/kodi-complete-testing-guide.md).
...
**Still matches (non-exhaustive, 2026-04-24):** `Includes_Images.xml`, `Includes_Overlay.xml`, `Includes_DialogInfo.xml`, `Includes_Search.xml`, `Includes_Widgets.xml`, `Includes_Views_Combined.xml`, `Custom_1140_OSD_Playlist.xml`, and related surfaces — see **[D-038](../../context/d038-legacy-properties-and-mapping.md)** for approved vs deferred symbols (e.g. discover folder path key, OSD crop, crew strip). (`Includes_Hubs.xml` is Velocity `list_id` wiring; grep it separately if re-auditing.)
