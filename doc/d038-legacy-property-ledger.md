...
| `1080i/Custom_1105_Search.xml` | 2 | helper-named discover property key | keep-temporary/replace | keep key temporarily if harmless; ensure target route is Velocity and plan rename later | locked-batch-d |
...
| `1080i/MyWeather.xml` | 1 | helper property branch in weather screen | remove | weather surface removed (never used in target fork) | locked-batch-d |
...
| `1080i/Custom_1161_Dialog_Weather.xml` | 1 | helper field in weather dialog | remove | weather dialog removed (never used in target fork) | locked-batch-d |
...
| `1080i/Custom_1180_Dialog_Bumper.xml` | 1 | helper field in bumper flow | keep-temporary/replace | low-risk surface; replace when bumper flow touched | locked-batch-d |
...
|---|---|---|---|
| `1080i/Includes_DialogInfo.xml` | `DialogInfo_VideoWidgets`, `DialogInfo_PersonWidgets`, `DialogInfo_VideoDetails`, `DialogInfo_CrewWidget`, and nested `TMDbHelper.*` branches | `DialogVideoInfo.xml` no longer passes `extended_include=DialogInfo_VideoDetails` and no longer includes person/video widget rails; these helper-heavy includes are now unreferenced by active details/OSD bridge flow. | Phase 06: remove inactive helper-heavy include blocks and related dead helper conditions. |
| `1080i/Includes_Paths.xml` | `Path_VideoInfo_Online*`, `Path_FromWriter`, `Path_FromDirector` helper-backed branches | Call sites now resolve only from inactive helper-heavy details rails in `Includes_DialogInfo.xml` (and legacy Batch A context surfaces), not from active Home/Series/Movies/provider/search/details/OSD bridge control-plane. | Phase 06: delete inactive helper-backed path variables after dead-code sweep. |
| `1080i/Includes_Actions.xml` | `Exp_TMDbHelper_IsBlur` conditions in scheme actions and `Action_TMDbHelper_Toggle_Onclick` legacy include name | Not on active navigation control-plane paths; exercised only via optional settings/customization surfaces outside current active-path enforcement scope. Helper blur onload/unload writes and ratings/service toggles already removed/nooped in Phase 03. | Phase 06: rename/remove residual helper-named action and replace remaining blur-expression conditions with non-helper expression names. |
| `1080i/Includes_Search.xml` + `1080i/Custom_1105_Search.xml` | `TMDbHelper.UserDiscover.FolderPath` key | Explicitly approved temporary key exception for combined search discover wiring; route target is Velocity (`plugin://plugin.video.velocity2/?action=discover`). | Phase 06: rename helper-named key to neutral search namespace without behavior change. |
